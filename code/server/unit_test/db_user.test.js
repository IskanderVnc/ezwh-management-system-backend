...
const db = require('../database/UserDB.js');
...
describe("USER - DB test", () => {
    ...
    afterAll(async () => {
        await db.deleteUserData();
        await db.createUser(new User(1, "user1name", "user1surname", "user1@ezwh.com", "testpassword", "customer"));
        await db.createUser(new User(2, "qualityEmployee1name", "qualityEmployee1surname", "qualityEmployee1@ezwh.com", "testpassword", "qualityEmployee"));
        await db.createUser(new User(3, "clerk1name", "clerk1surname", "clerk1@ezwh.com", "testpassword", "clerk"));
        await db.createUser(new User(4, "deliveryEmployee1name", "deliveryEmployee1surname", "deliveryEmployee1@ezwh.com", "testpassword", "deliveryEmployee"));
        await db.createUser(new User(5, "supplier1name", "supplier1surname", "supplier1@ezwh.com", "testpassword", "supplier"));
        await db.createUser(new User(6, "manager1name", "manager1surname", "manager1@ezwh.com", "testpassword", "manager"));
    });
...
function testUpdateUser(email, type, call) {
    test('update User', async () => {
        if (call <= 2) {
            // SUCCESSFULL CALLS
            let res = await db.updateUser(email, type);
            expect(res).toStrictEqual(true);
        } else {
            // CALL WITH EXPECTED ERROR IN DATABASE (ELEMENT CREATION WITH NOT NULL CONSTRAINT NOT SATISIFED)
            try {
                await db.updateUser(email, type);
            } catch (err) {
                let res = false;
                expect(res).toStrictEqual(false);
            }
...
function testDeleteUser(email, type, call) {
    test('delete User', async () => {
        let res = await db.deleteUser(email, type);
        if (call <= 2) {
            expect(res).toStrictEqual(true);
        }
        else {
...
function testGetUserInfo(username, call) {
    test('get userInfo', async () => {
        ...
        switch (call) {
            case 1:
                res = await db.getUserInfo(username);
                expect(res).toEqual({
                    id: 1,
                    name: "Marco",
                    surname: "Rossi",
...
function testSupplierList() {
    test('get supplier list', async () => {
        let res = await db.getSupplierList();
        expect(res).toEqual([{
            id: 2,
            name: "Luca",
            surname: "Verdi",
...
function testGetUserByEmailAndPassword(username, password, call) {
    test('get User By Email And Password', async () => {
        ...
        switch (call) {
            case 1:
                res = await db.getUserByEmailAndPassword(username, password);
                expect(res).toEqual({
                    id: 1,
                    name: "Marco",
                    surname: "Rossi",
...
function testGetUserByEmailAndType(email, type, call) {
    test('get User By email And type', async () => {
        ...
        switch (call) {
            case 1:
                res = await db.getUserByEmailAndType(email, type);
                expect(res).toEqual({
                    id: 1,
                    name: "Marco",
                    surname: "Rossi",
