...
const returnOrder_service = new ReturnOrderService(ReturnOrderDB);
const Return_Order = require('../models/Return_Order');
const Product_Return = require('../models/Product_Return');


describe('Return Order Service - DB test', () => {

    beforeAll(async () => {
        await returnOrder_service.deleteReturnOrderData();
        await returnOrder_service.deleteItemsToReturnData();
    });

    test('Checking if no return orders are present', async () => {
        var res = await returnOrder_service.getReturnOrderList();
    ...
    testContentGetProductsToReturnOrderByID(1);
    testContentGetProductsToReturnOrderByID(2);
    testContentGetProductsToReturnOrderByID(3);
    testContentGetProductsToReturnOrderByID(4);
...
function testNewReturnOrder(returnOrder, call) {
    test('Create new Return Order', async () => {
        ...
        if (call <= 3) {
            // SUCCESSFULL CALLS
            let res = await returnOrder_service.createReturnOrder(returnOrder);
...
function testAddSkuItemToskuReturnTable(product, returnOrderID, call) {
    test('Add products to SkuItemsToReturn table', async () => {
        if (call <= 4) {
            // SUCCESSFULL CALLS 
            let res = await returnOrder_service.addSkuItemToSkuReturnTable(product, returnOrderID);
            expect(res).toStrictEqual(true);
        } else {
            try {
                await returnOrder_service.addSkuItemToSkuReturnTable(product, returnOrderID);
            } catch (err) {
                let res = false;
                expect(res).toStrictEqual(false);
            }
...
function testNumberGetProductsToReturnOrderByID(id, n) {
    ...
    test('Get Products of a Return Order by ID - Check of quantity of found products ', async () => {
        if (n != 0) {
            let products = await returnOrder_service.getProductstoReturnOrder(id);
...
function testContentGetProductsToReturnOrderByID(id) {
    test('Get Products of a Return Order by ID - Check of content of found products', async () => {
        ...
        switch (id) {
            case 1:
                res = await returnOrder_service.getProductstoReturnOrder(id);
...
function testDeleteReturnOrderByID(id) {
    test('Delete Return Order by ID', async () => {
        let res = await returnOrder_service.deleteReturnOrderById(id);
        if (id <= 3) {
            expect(res).toStrictEqual(true);
        }
        else {
