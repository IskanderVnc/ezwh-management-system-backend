...
const db = require('../database/RestockOrderDB');

describe('Restock Order - DB test', () => {

    beforeAll(async () => {
        await db.deleteRestockOrderData();
        await db.deleteSKUItemToRestockData();
        await db.deleteSKUToRestockData();
    });

    test('Checking if no Restock orders are present', async () => {
        var res = await db.getRestockOrderList();
    ...
    // TESTING addProductsToOrder by ID (id) - addProductsToOrder is a function that returns an array of products taken from SKUToRestock which is used in APIs to add the array into the restockOrder Object
...
function testNewRestockOrder(RestockOrder, call) {
    test('Create new Restock Order', async () => {
        if (call <= 2) {
            // SUCCESSFULL CALLS
            let res = await db.createRestockOrder(RestockOrder);
            ...
            try {
                await db.createRestockOrder(RestockOrder);
            } catch (err) {
                let res = false;
...
function testGetRestockOrderByID(id) {
    test('Get Restock Order by ID', async () => {
        ...
        switch (id) {
            case 1:
                res = await db.getRestockOrderbyId(id);
                expect(res).toEqual({
                    id: 1,
...
function testAddSkuItemToskuRestockTable(product, restockOrderID, call) {
    test('Add products to SkuToRestock table', async () => {
        if (call <= 4) {
            // SUCCESSFULL CALLS 
            let res = await db.addProductToSKURestockTable(product, restockOrderID);
...
function testAddSkuItemToSkuItemToRestockTable(item, restockOrderID, call) {
    test('Add items to SkuToRestock table', async () => {
        if (call <= 4) {
            // SUCCESSFULL CALLS 
            let res = await db.addSkuItemsToSkuItemToRestock(item, restockOrderID);
            expect(res).toStrictEqual(true);
        } else {
            // CALLS WITH DB ERROR CAUSE BY FAILED CONTSTRAINTS
            try {
                await db.addSkuItemsToSkuItemToRestock(item, restockOrderID);
            } catch (err) {
                let res = false;
                expect(res).toStrictEqual(false);
            }
...
function testNumberGetSkuItemsToRestockOrder(id, n) {
    test('Get sku items  of a Restock Order by ID - Check quantity of found items ', async () => {
        if (n != 0) {
            let skuItems = await db.getSkuItemstoOrder(id);
...
function testContentGetSkuItemsToRestockOrder(id) {
    test('Get Products of a Restock Order by ID - Check content of found products', async () => {
        ...
        switch (id) {
            case 1:
                res = await db.getSkuItemstoOrder(id);
...
function testUpdateRestockOrderState(restockOrder, restockOrderID) {
    test('Update Restock Order State by Id', async () => {
        if (restockOrderID == 1) {
            // SUCCESSFULL CALL
            let res = await db.updateRestockOrderState(restockOrder, restockOrderID);
            expect(res).toStrictEqual(true);
        } else if (restockOrderID == 2) {
            // CALL WITH NOT NULL CONSTRAINT FAILED 
            try {
                await db.updateRestockOrderState(restockOrder, restockOrderID);
            } catch (err) {
                let res = false;
                expect(res).toStrictEqual(false);
            }
...
function testUpdateRestockOrderTransportNote(restockOrder, restockOrderID) {
    test('Update Restock Order Transport Note by Id', async () => {
        if (restockOrderID == 1) {
            // SUCCESSFULL CALL
            let res = await db.updateRestockOrderTransportNote(restockOrder, restockOrderID);
            expect(res).toStrictEqual(true);
        } else if (restockOrderID == 2) {
            // CALL WITH NOT NULL CONSTRAINT FAILED 
            try {
                await db.updateRestockOrderTransportNote(restockOrder, restockOrderID);
            } catch (err) {
                let res = false;
                expect(res).toStrictEqual(false);
            }
...
function testAddProductsToOrder(id) {
    test('Taking Products of a restock Order By ID from SKUtoRestock table', async () => {
        ...
        switch (id) {
            case 1:
                res = await db.addProductstoOrder(id);
                expect(res[0]).toEqual({
                    price: 9.99,
                    SKUId: 1,
                    qty: 24,
...
function testDeleteRestockOrderByID(id) {
    test('Delete Restock Order by ID', async () => {
        let res = await db.deleteRestockOrderById(id)
        if (id <= 2) {
            expect(res).toStrictEqual(true);
        }
        else {
